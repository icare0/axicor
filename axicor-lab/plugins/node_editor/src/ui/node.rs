...
pub fn calc_all_layouts(
    ...
    for (nodes, node_type, default_y) in groups {
        for (i, zone) in nodes.iter().enumerate() {
            ...
            let local_pos = *state.node_positions.entry(zone.clone()).or_insert_with(|| {
            ...
            layouts.insert(
                zone.clone(),
                calc_node_layout(node_type, local_pos, &inputs, &outputs, transform),
            );
        }
    }
            ...
            if edit.lost_focus() {
                if ui.input(|i| i.key_pressed(egui::Key::Escape)) {
                    ...
                    if !new_name.is_empty() && new_name != zone {
                        send_mutation(TopologyMutation::Rename(
                            crate::domain::RenameTarget::Shard {
...
fn draw_node_shape(
    ...
    let (header_color, border_base) = match node_type {
        NodeType::EnvRX => (
            Color32::from_rgb(35, 65, 45),
            Color32::from_rgb(50, 160, 80),
        ),
        NodeType::EnvTX => (
            Color32::from_rgb(65, 35, 35),
...
fn handle_node_drag(
    ...
    if response.secondary_clicked() {
        if let Some(pos) = ui.ctx().pointer_hover_pos() {
            let label_suffix = if state.level == crate::domain::EditorLevel::Model {
                "Department"
            } else {
                "Shard"
            };
...
fn draw_input_pins(
    ...
    for port in inputs {
        let Some(&pin_pos) = layout.input_pins.get(port) else {
            continue;
        };
        draw_pin(painter, pin_pos, r, CLR_PIN_IN);

        let is_editing =
            state.renaming_port.as_ref() == Some(&(zone.to_string(), true, port.clone()));
        if is_editing {
            let edit_rect = Rect::from_min_size(
                pin_pos + Vec2::new(10.0 * zoom, -8.0 * zoom),
                Vec2::new(60.0 * zoom, 16.0 * zoom),
            );
                ...
                if edit.lost_focus() {
                    if ui.input(|i| i.key_pressed(egui::Key::Escape)) {
                        ...
                        if !new_name.is_empty() && new_name != *port {
                            send_mutation(TopologyMutation::Rename(
                                crate::domain::RenameTarget::IoPin {
                                    zone: zone.to_string(),
                                    is_input: true,
                                    old_name: port.clone(),
                                    new_name,
        ...
        if resp.secondary_clicked() {
            if port == "in" {
                ...
                send_context_menu(layout_api::OpenContextMenuEvent {
                    ...
                    actions: vec![
                        layout_api::MenuAction {
                            action_id: format!("node_editor.start_rename_port|{}|1|{}", zone, port),
                            label: " Rename Port".into(),
                        },
                        layout_api::MenuAction {
                            action_id: format!("node_editor.delete_port|{}|1|{}", zone, port),
                            label: " Delete Port".into(),
                        },
                    ],
                });
            }
    ...
    painter.text(
        plus_pos + Vec2::new(10.0 * zoom, 0.0),
        egui::Align2::LEFT_CENTER,
        "+",
        egui::FontId::proportional(12.0 * zoom),
        Color32::DARK_GRAY,
    ...
    if ui
        .interact(
            Rect::from_center_size(plus_pos, Vec2::splat(r * 2.0)),
            ui.id().with((zone, "add_in")),
            egui::Sense::click(),
        )
        .clicked()
    {
        send_mutation(TopologyMutation::Create(
            crate::domain::CreateTarget::IoPin {
                zone: zone.to_string(),
                is_input: true,
                name: format!("in_{}", inputs.len() + 1),
            },
            None,
        ));
    }
...
fn draw_output_pins(
    ...
    for port in outputs {
        let Some(&pin_pos) = layout.output_pins.get(port) else {
            continue;
        };
        draw_pin(painter, pin_pos, r, CLR_PIN_OUT);

        let is_editing =
            state.renaming_port.as_ref() == Some(&(zone.to_string(), false, port.clone()));
        if is_editing {
            let edit_rect = Rect::from_min_max(
                pin_pos - Vec2::new(70.0 * zoom, 8.0 * zoom),
                pin_pos - Vec2::new(10.0 * zoom, -8.0 * zoom),
            );
            ui.allocate_ui_at_rect(edit_rect, |ui| {
        ...
        if out_response.drag_started() || out_response.clicked() {
            state.dragging_pin = Some((zone.to_string(), port.clone(), pin_pos, false));
        }
        ...
        if out_response.secondary_clicked() {
            if port == "out" {
                ...
                send_context_menu(layout_api::OpenContextMenuEvent {
                    ...
                    actions: vec![
                        layout_api::MenuAction {
                            action_id: format!("node_editor.start_rename_port|{}|0|{}", zone, port),
                            label: " Rename Port".into(),
                        },
                        layout_api::MenuAction {
                            action_id: format!("node_editor.delete_port|{}|0|{}", zone, port),
                            label: " Delete Port".into(),
                        },
                    ],
                });
        ...
        .clicked()
    {
        send_mutation(TopologyMutation::Create(
            crate::domain::CreateTarget::IoPin {
                zone: zone.to_string(),
                is_input: false,
                name: format!("out_{}", outputs.len() + 1),
            },
            None,
        ));
    }
