use crate::domain::{BrainTopologyGraph, EditorLevel, NodeGraphUiState, ProjectSession};
use bevy::prelude::*;
...
pub fn spawn_load_task_system(
    ...
    for ev in load_events.read() {
        ...
        let path = PathBuf::from("Axicor-Models")
            .join(&name)
            .join("simulation.toml");
        if graph.sessions.get(&path).map_or(false, |s| s.is_dirty) {
            info!(
                "[NodeEditor] Session '{:?}' is dirty. Skipping reload.",
                path
            );
            continue;
        }
    ...
    for ev in topo_events.read() {
        target_path = Some(
            PathBuf::from("Axicor-Models")
                .join(clean_proj_name(&ev.project_name))
                .join("brain.toml"),
        );
    }
    ...
    info!(
        "[NodeEditor] Spawning load task for: {:?} at level {:?}",
        path, level
    );
    let cache_clone = ProjectFsCache {
        projects: fs_cache.projects.clone(),
    };
    let task = AsyncComputeTaskPool::get()
        .spawn(async move { load_graph_from_disk(path, level, cache_clone) });
...
pub fn apply_loaded_graph_system(
    ...
    for (entity, mut task) in tasks.iter_mut() {
        ...
        let session = graph
            ...
            .or_insert_with(ProjectSession::default);
    ...
    if let Ok(content) = layout_api::overlay_read_to_string(&path) {
        if let Ok(doc) = content.parse::<toml_edit::DocumentMut>() {
            ...
            for zone in &zones {
                            ...
                            .and_then(|i| i.as_array_of_tables())
                        {
                            for table in arr.iter() {
                                ...
                                let pct = table
                                    ...
                                    .unwrap_or(1.0)
                                    as f32;
                                layers.push(crate::domain::ShardLayer {
                                    name: l_name,
                                    height_pct: pct,
                                });
                            }
                ...
                shard_anatomies.insert(
                    zone.clone(),
                    crate::domain::ShardAnatomy { w, d, h, layers },
                );

                let blueprints_path = shard_path
                    .parent()
                    .unwrap_or(Path::new("."))
            ...
            let layout_path = path.parent().unwrap().join(format!(
                "{}.layout.toml",
                path.file_name()
                    .unwrap()
                    .to_string_lossy()
