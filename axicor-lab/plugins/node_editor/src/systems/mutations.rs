    ...
    for ev in events.read() {
        match ev {
            ...
            TopologyMutation::Delete(target, context_path) => {
                ...
                let Some(session) = graph.sessions.get_mut(&active_path) else {
                    continue;
                };
                ...
                match target {
                    crate::domain::DeleteTarget::Zone { name, id } => {
                        ...
                        info!(
                            "[RAM Sync] Evicted node {} from session {:?}",
                            name, active_path
                        );
                    }
                    crate::domain::DeleteTarget::Connection {
                        ...
                        session.connections.retain(|(f, fp, t, tp)| {
                    ...
                    crate::domain::DeleteTarget::IoPin {
                        ...
                        if *is_input {
                            if let Some(inputs) = session.node_inputs.get_mut(zone.as_str()) {
                                inputs.retain(|p| p != name);
                            }
                        } else {
                            if let Some(outputs) = session.node_outputs.get_mut(zone.as_str()) {
                                outputs.retain(|p| p != name);
                            }
                        }
                        session.connections.retain(|(f, fp, t, tp)| {
                            !((f == zone && fp == name && !*is_input)
                                || (t == zone && tp == name && *is_input))
                        });
                        info!(
                            "[RAM Sync] Removed I/O Matrix '{}' from zone '{}'",
                            name, zone
                        );
                    }
            ...
            TopologyMutation::Rename(target, context_path) => {
                ...
                match target {
                    crate::domain::RenameTarget::Shard {
                        ...
                        if let Some(pos) = session.env_rx_nodes.iter().position(|z| z == old_name) {
                            session.env_rx_nodes[pos] = new_name.clone();
                            found = true;
                        }
                        if let Some(pos) = session.env_tx_nodes.iter().position(|z| z == old_name) {
                            session.env_tx_nodes[pos] = new_name.clone();
                            found = true;
                        }
                        ...
                        for conn in session.connections.iter_mut() {
                            if conn.0 == *old_name {
                                conn.0 = new_name.clone();
                            }
                            if conn.2 == *old_name {
                                conn.2 = new_name.clone();
                            }
                        }
                    ...
                    crate::domain::RenameTarget::IoPin {
                        ...
                        if *is_input {
                            if let Some(inputs) = session.node_inputs.get_mut(zone) {
                                if let Some(pos) = inputs.iter().position(|p| p == old_name) {
                                    inputs[pos] = new_name.clone();
                                }
                            }
                        } else {
                            if let Some(outputs) = session.node_outputs.get_mut(zone) {
                                if let Some(pos) = outputs.iter().position(|p| p == old_name) {
                                    outputs[pos] = new_name.clone();
                                }
                            }
                        }
                        for conn in session.connections.iter_mut() {
                            if conn.0 == *zone && conn.1 == *old_name && !*is_input {
                                conn.1 = new_name.clone();
                            }
                            if conn.2 == *zone && conn.3 == *old_name && *is_input {
                                conn.3 = new_name.clone();
                            }
                        }
                        session.is_dirty = true;
                        info!(
                            "[RAM Sync] Renamed IO Pin {} to {} in {}",
                            old_name, new_name, zone
                        );
                    }
                }
            }
