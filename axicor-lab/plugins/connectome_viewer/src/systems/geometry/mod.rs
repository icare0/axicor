...
use crate::domain::{NeuronInstances, ShardGeometry, ZoneSelectedEvent};
use crate::systems::material::NeuronInstanceMaterial;
use bevy::pbr::{MaterialMeshBundle, PbrBundle};
use bevy::prelude::*;
use bevy::render::view::{NoFrustumCulling, RenderLayers};
use layout_api::{PluginInput, PluginWindow};
use tracing::error;
...
pub fn load_zone_geometry_system(
    ...
    for ev in events.read() {
        ...
        let axic_path =
            std::path::PathBuf::from("Axicor-Models").join(format!("{}.axic", ev.project_name));
        ...
        if let Some(paths_bytes) = archive.get_file(&paths_path) {
            ...
            if let Some(axon_result) = axons::build_axon_lines(
                paths_bytes,
                pos_data,
                soma_result.center,
