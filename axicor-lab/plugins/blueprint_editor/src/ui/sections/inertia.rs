...
pub fn draw_inertia_section(ui: &mut egui::Ui, nt: &mut NeuronType) -> bool {
    ...
    egui::CollapsingHeader::new(
        egui::RichText::new("Inertia Curve")
            .strong()
    ...
    .show(ui, |ui| {
        ui.horizontal(|ui| {
            ...
            for i in 0..16 {
                ...
                // Single column
                let (rect, resp) =
                    ui.allocate_exact_size(egui::vec2(12.0, 40.0), egui::Sense::click_and_drag());

                // Gradient: Plastic (Blue) -> Monumental (Red)
                let t = i as f32 / 15.0;
                let color =
                    egui::Color32::from_rgb((t * 255.0) as u8, 80, ((1.0 - t) * 255.0) as u8);
                ...
                ui.painter()
                    .rect_filled(rect, 0.0, egui::Color32::from_black_alpha(50)); // Channel background
                ui.painter().rect_filled(bar_rect, 1.0, color); // Visualization
            }
        });
        ui.label(
            egui::RichText::new("Rank 0 (Plastic)  Rank 15 (Monumental)")
