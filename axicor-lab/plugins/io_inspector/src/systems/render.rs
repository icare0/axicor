                    ...
                    egui::Frame::none()
                        ...
                        .show(ui, |ui| {
                            let Some(active_path) = graph.active_path.as_ref() else {
                                ui.centered_and_justified(|ui| { ui.label(egui::RichText::new("No active project").color(egui::Color32::DARK_GRAY)); });
                                return;
                            };
                            ...
                            if zone.is_empty() {
                                ui.centered_and_justified(|ui| { ui.label(egui::RichText::new("Select a Shard in Node Editor").color(egui::Color32::DARK_GRAY)); });
                                return;
                            }
                            ...
                            egui::ScrollArea::vertical().show(ui, |ui| {
                                if let Some(io) = io_data {
                                    ...
                                    if !io.input.is_empty() {
                                        ui.label(egui::RichText::new("INPUTS").color(egui::Color32::from_rgb(100, 200, 255)).small());
                                    ...
                                    if !io.output.is_empty() {
                                        ui.label(egui::RichText::new("OUTPUTS").color(egui::Color32::from_rgb(255, 150, 50)).small());
                                    ...
                                    ui.centered_and_justified(|ui| { ui.label(egui::RichText::new("No I/O Data available").color(egui::Color32::DARK_GRAY)); });
                                }
                            });
                        });
                });
...
fn draw_capsule(ui: &mut egui::Ui, zone: &str, name: &str, is_input: bool) {
    ...
    if response.dragged() { ui.ctx().set_cursor_icon(egui::CursorIcon::Grabbing); }
    if response.drag_stopped() { ui.ctx().memory_mut(|m| m.data.remove_temp::<IoWirePayload>(payload_id)); }
}
...
fn render_flying_wire(ctx: &egui::Context) {
    ...
    if let Some(payload) = ctx.memory(|m| m.data.get_temp::<IoWirePayload>(payload_id)) {
        if let Some(mouse_pos) = ctx.input(|i| i.pointer.latest_pos()) {
            ...
            let end = mouse_pos;
            painter.line_segment([start, end], egui::Stroke::new(2.5, egui::Color32::from_white_alpha(150)));
