use crate::domain::{GitStatus, ProjectModel, ProjectNode, ProjectNodeType, ProjectStatus};
use bevy::prelude::*;
use std::collections::HashMap;
use std::fs;
use std::path::Path;

pub fn fs_scanner_system(mut cache: ResMut<crate::domain::ProjectFsCache>) {
            ...
            if entry.path().is_dir() {
                ...
                if sim_path.exists() {
                    ...
                    let sim_node_hot = ProjectNode {
                        id: extract_id_overlay(&sim_path, "model_id_v1")
                    ...
                    sim_node_cold.children =
                        merge_nodes(sim_node_cold.children, sim_node_hot.children);
...
fn scan_shards(model_path: &Path, brain_path: &Path, is_hot: bool) -> Vec<ProjectNode> {
    ...
    if let Ok(toml_val) = content.parse::<toml::Value>() {
        ...
        error!(
            "[ERROR] [Scanner] Failed to parse TOML at {:?}. Check for syntax errors.",
            brain_path
        );
    }
    children
...
fn merge_nodes(cold: Vec<ProjectNode>, hot: Vec<ProjectNode>) -> Vec<ProjectNode> {
    ...
    let mut hot_map: HashMap<String, ProjectNode> =
        hot.into_iter().map(|n| (n.id.clone(), n)).collect();
    ...
    merged.sort_by(|a, b| {
        ...
        type_ord(&a.git_status)
            .cmp(&type_ord(&b.git_status))
