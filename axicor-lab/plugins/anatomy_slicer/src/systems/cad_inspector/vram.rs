...
pub fn allocate_vram_system(
    ...
    for mut state in query.iter_mut() {
        ...
        info!(
            "[VRAM] Allocated {}x{} for CAD Inspector",
            target_w, target_h
        );
    }
}

pub fn sync_vram_system(query: Query<&AnatomySlicerState>, mut images: ResMut<Assets<Image>>) {
    for state in query.iter() {
        if let Some(handle) = &state.shard_rtt {
            ...
            if let Some(image) = images.get(handle) {
                if image.texture_descriptor.size.width != target_w
