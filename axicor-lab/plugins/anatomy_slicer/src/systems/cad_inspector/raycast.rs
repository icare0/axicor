...
pub fn dnd_raycast_system(
    ...
    let Some((window_entity, mut state)) = ui_states.iter_mut().find(|(_, s)| s.active_zone.is_some()) else { return };
                ...
                if rect.contains(mouse_pos) {
                    ...
                    if let Some(ray) = camera.viewport_to_world(cam_transform, bevy::math::Vec2::new(local_pos.x, local_pos.y)) {
                        if let Some((hit_pos, voxel_z)) = intersect_shard(ray, w, h, d) {
                            ...
                            let snap_world_pos = Vec3::new(if payload.is_input { -w/2.0 } else { w/2.0 }, center_y, hit_pos.z);
                            
                            if let Some(viewport_pos) = camera.world_to_viewport(cam_transform, snap_world_pos) {
                                let screen_snap = bevy_egui::egui::Pos2::new(rect.min.x + viewport_pos.x, rect.min.y + viewport_pos.y);
