...
use layout_api::{base_domain, draw_unified_header, PluginWindow, DOMAIN_ANATOMY_SLICER};
...
pub fn render_anatomy_slicer_system(
    ...
    for (entity, window) in windows.iter() {
        if !window.is_visible || base_domain(&window.plugin_id) != DOMAIN_ANATOMY_SLICER {
            continue;
        }
                            ...
                            .show(ui, |ui| {
                                let Some(zone) = &state.active_zone else {
                                    ui.centered_and_justified(|ui| {
                                        ui.label(
                                            egui::RichText::new("No Shard Selected")
                                ...
                                let Some(anatomy) = session.shard_anatomies.get(zone) else {
                                    return;
                                };

                                egui::ScrollArea::vertical().show(ui, |ui| {
                                    ui.label(
                                        egui::RichText::new(format!("ANATOMY: {}", zone))
