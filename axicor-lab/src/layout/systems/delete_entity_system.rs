use crate::layout::systems::wm_file_ops::{
    load_document, remove_array_of_tables_item, remove_connection_record, remove_io_record_by_name,
    save_document,
};
use bevy::prelude::*;
use node_editor::domain::{BrainTopologyGraph, DeleteTarget, TopologyMutation};
use std::path::Path;
        ...
        if let TopologyMutation::Delete(target, context_path) = ev {
            ...
            match target {
                ...
                DeleteTarget::IoPin {
                    ...
                    if let Ok(mut doc) =
                        crate::layout::systems::wm_file_ops::load_document(&io_path)
                    {
                        if crate::layout::systems::wm_file_ops::remove_io_record_by_name(
                            &mut doc, section, name,
                        ) {
                            let _ =
                                crate::layout::systems::wm_file_ops::save_document(&io_path, &doc);
                            ...
                            if let Some(session) = graph.sessions.get_mut(active_path) {
                                ...
                                if let Some(io_data) = session.shard_io.get_mut(zone) {
                                    if let Ok(new_io) =
                                        toml::from_str::<node_editor::domain::ShardIoData>(
                                            &doc.to_string(),
                                        )
                                    {
                                        *io_data = new_io;
                            ...
                            if let Ok(mut macro_doc) = load_document(active_path) {
                                    ...
                                    .and_then(|i| i.as_array_of_tables())
                                {
                                    for (i, table) in arr.iter().enumerate() {
                                        let f = table
                                            ...
                                            .unwrap_or("");
                                        let t =
                                            table.get("to").and_then(|v| v.as_str()).unwrap_or("");
                                        let out_m = table
                                            .get("output_matrix")
                                            .and_then(|v| v.as_str())
                                            .unwrap_or("");
                                ...
                                if !connections_to_remove.is_empty() {
                                    ...
                                    //   ,
                                    for (idx, target_zone, w, h) in
                                        connections_to_remove.into_iter().rev()
                                    {
                                        if let Some(arr) = macro_doc
                                            .get_mut("connection")
                                            .and_then(|i| i.as_array_of_tables_mut())
                                        {
                                            ...
                                            //  ghost_capacity
                                            let dst_shard_path = if is_sim {
                                                project_dir.join(&target_zone).join("shard.toml")
                                            } else {
                                                project_dir
                                                    .join(&dept_name)
                                                    .join(&target_zone)
                                            ...
                                            if let Ok(mut dst_doc) = load_document(&dst_shard_path)
                                            {
                                                ...
                                                if let Some(settings) = dst_doc
                                                    .get_mut("settings")
                                                    .and_then(|s| s.as_table_mut())
                                                {
                                                    settings.insert(
                                                        "ghost_capacity",
                                                        toml_edit::value(new_cap),
                                                    );
                                                    let _ =
                                                        save_document(&dst_shard_path, &dst_doc);
...
fn delete_shard(
    ...
    name: &str,
    id: &str,
    _deleted_ev: &mut EventWriter<layout_api::EntityDeletedEvent>,
    _fs_cache: &layout_api::ProjectFsCache,
) {
    info!(
        " [Orchestrator] Starting physical deletion of Shard: {} (ID: {})",
        name, id
    );
    ...
    info!(
        " [Sandbox] Shard {} removed from AST. Physical deletion deferred to Compile phase.",
        name
    );
}
...
fn delete_department(
    ...
    info!(
        " [Sandbox] Department {} removed from AST. Physical deletion deferred to Compile phase.",
        name
    );
}
...
fn delete_connection(
    ...
    let (is_from_rx, is_to_tx, from_id, to_id) =
