...
pub fn route_plugin_inputs_system(
    ...
    for (window, mut input) in query.iter_mut() {
        let Some(rect) = allocated.rects.get(&window.plugin_id) else {
            continue;
        };

        let is_hovered = pointer_pos.is_some_and(|p| rect.contains(p));

        // LAW: button state only updates if hovered or already pressed
        // (drag-out: allows tracking movement outside the tile if capture began inside)
        input.is_primary_pressed =
            pointer.primary_down() && (is_hovered || input.is_primary_pressed);
        input.is_secondary_pressed =
            pointer.secondary_down() && (is_hovered || input.is_secondary_pressed);
        input.is_middle_pressed = pointer.middle_down() && (is_hovered || input.is_middle_pressed);

        if is_hovered {
            let pos = pointer_pos.unwrap(); // safe: is_hovered implies Some
