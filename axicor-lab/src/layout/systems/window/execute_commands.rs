...
use bevy_egui::egui;
use egui_tiles::{Container, Linear, SimplificationOptions, Tile};

use crate::layout::domain::{Pane, TreeCommands, WorkspaceState};
use layout_api::{base_domain, domain_title, PluginWindow, TopologyCache, TreeCommand};
    ...
    if let Some(tree) = workspace.trees.get_mut(&active_ws) {
        for cmd in commands_queue.queue.drain(..) {
            match cmd {
                ...
                TreeCommand::Merge { survivor, victim } => handle_merge(tree, survivor, victim),

                TreeCommand::SwapPanes { src, dst } => handle_swap(tree, src, dst),

                TreeCommand::ChangeDomain {
                    tile_id,
                    new_domain,
...
fn handle_split(
    ...
    // Unique ID via elapsed - does not break ECS contract unlike SystemTime
    let new_plugin_id = format!("{}::{}", base, time.elapsed().as_micros());
    let pane = Pane {
        plugin_id: new_plugin_id.clone(),
        title: domain_title(base).to_string(),
    };
    ...
    let Some(Tile::Pane(old_pane)) = tree.tiles.get(target).cloned() else {
        return;
    };
    ...
    tree.tiles
        .insert(target, Tile::Container(Container::Linear(linear)));
}

fn handle_merge(
    tree: &mut egui_tiles::Tree<Pane>,
    survivor: egui_tiles::TileId,
    victim: egui_tiles::TileId,
) {
    // O(N) parent map
    let parent_map = build_parent_map(&tree.tiles);

    let Some(&victim_parent) = parent_map.get(&victim) else {
        return;
    };

    // Lifting survivor to the direct neighbor level of the victim
...
fn handle_swap(
    tree: &mut egui_tiles::Tree<Pane>,
    src: egui_tiles::TileId,
    dst: egui_tiles::TileId,
) {
    let (Some(Tile::Pane(src_pane)), Some(Tile::Pane(dst_pane))) =
        (tree.tiles.get(src).cloned(), tree.tiles.get(dst).cloned())
    else {
        return;
    };

    if let Some(Tile::Pane(p)) = tree.tiles.get_mut(src) {
        *p = dst_pane;
    }
    if let Some(Tile::Pane(p)) = tree.tiles.get_mut(dst) {
        *p = src_pane;
    }
}

fn handle_change_domain(
    tree: &mut egui_tiles::Tree<Pane>,
    ...
    let pane = Pane {
        plugin_id: new_domain.to_string(),
        title: domain_title(base).to_string(),
    };
...
) -> bevy::utils::HashMap<egui_tiles::TileId, egui_tiles::TileId> {
...
pub fn spawn_pane_entity(
    ...
    // DOD FIX: WM only spawns the shell.
    // Textures are allocated in sync_plugin_geometry_system.
    // Plugins themselves are responsible for spawning their specific logic (e.g., Camera3d).
    commands
        .spawn((
            PluginWindow {
...
pub fn create_plugin_render_target(
    images: &mut Assets<Image>,
    width: u32,
    height: u32,
) -> Handle<Image> {
