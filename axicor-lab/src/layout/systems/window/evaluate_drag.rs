...
use crate::layout::domain::{Pane, TreeCommands, WindowDragState, WorkspaceState};
use layout_api::{DragIntent, DragSource, TopologyCache, TreeCommand, WindowDragRequest};
...
pub fn evaluate_drag_intents_system(
    ...
    // ---   ---
    if !drag_state.is_dragging {
        let Some(src_tile) = drag_request.target_tile else {
            return;
        };
    ...
    let (Some(src_tile), Some(start_pos)) = (drag_state.source_tile, drag_state.start_pos) else {
        return;
    };
    let Some(&src_rect) = topology.tiles.get(&src_tile) else {
        return;
    };
    ...
    if drag_request.source == DragSource::EdgeTrigger
        && drag_state.drag_axis.is_none()
        && delta.length_sq() > AXIS_LOCK_DIST_SQ
    {
        let horizontal = delta.x.abs() > delta.y.abs();
        drag_state.drag_axis = Some(if horizontal {
            LinearDir::Horizontal
        } else {
            LinearDir::Vertical
        });
        drag_state.drag_normal = Some(if horizontal {
            delta.x.signum()
        } else {
            delta.y.signum()
        });
    ...
    let Some(tree) = workspace.trees.get(&active_ws) else {
        return;
    };
...
fn flush_drag_command(drag_state: &WindowDragState, queue: &mut TreeCommands) {
    if matches!(drag_state.intent, DragIntent::None) {
        return;
    }
    ...
    let cmd = match &drag_state.intent {
        ...
        DragIntent::Merge { victim } => TreeCommand::Merge {
            survivor: src,
            victim: *victim,
        },
        DragIntent::Swap { victim } => TreeCommand::SwapPanes { src, dst: *victim },
        _ => return,
    };
...
fn compute_merge_intent(
    ...
    let victim = topology.tiles.iter().find(|(&id, r)| {
        ...
        if !r.expand(MERGE_HIT_EXPAND).contains(current_pos) {
            return false;
        }
...
fn compute_swap_intent(
    ...
    let victim = topology
        .tiles
        .iter()
        .find(|(&id, r)| id != src_tile && r.contains(current_pos));
    match victim {
