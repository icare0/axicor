...
use crate::layout::domain::{
    OsWindowCommand, SaveDefaultLayoutEvent, TreeCommands, WindowDragState, WorkspaceState,
};
use bevy::prelude::*;
use bevy_egui::{egui, EguiContexts};
use layout_api::{AllocatedPanes, CreateNewModelEvent, TopologyCache, WindowDragRequest};
        ...
        .show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                let (logo_rect, logo_resp) =
                    ui.allocate_exact_size(egui::vec2(logo_size, logo_size), egui::Sense::click());
                ...
                for ws in order {
                    ...
                    if workspace.renaming_workspace.as_ref() == Some(&ws) {
                        let res = ui.add(
                            egui::TextEdit::singleline(&mut workspace.rename_buffer)
                                .desired_width(80.0),
                        );
                        if res.lost_focus() {
                        ...
                        let resp = ui.selectable_label(
                            is_active,
                            egui::RichText::new(&ws).strong().color(color),
                        );
                ...
                if let Some(ws) = tab_to_remove {
                    ...
                    if workspace.active_workspace == ws {
                        workspace.active_workspace = workspace
                            .workspace_order
                            .first()
                            .cloned()
                            .unwrap_or_default();
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ...
                    if ui.add(egui::Button::new(" [] ").frame(false)).clicked() {
                        os_cmd.send(OsWindowCommand::Maximize);
                    }
                    if ui.add(egui::Button::new(" _ ").frame(false)).clicked() {
                        os_cmd.send(OsWindowCommand::Minimize);
                    }

                    let rect = ui.available_rect_before_wrap();
                    if ui
                        .interact(rect, ui.id().with("drag_area"), egui::Sense::drag())
                        .drag_started()
                    {
