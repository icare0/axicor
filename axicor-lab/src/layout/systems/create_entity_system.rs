use crate::layout::systems::wm_file_ops::{add_io_record, load_document, save_document};
use axicor_core::config::sys::SystemMeta;
use bevy::prelude::*;
use node_editor::domain::{BrainTopologyGraph, CreateTarget, TopologyMutation};
use std::fs;
use std::path::Path;
...
pub fn create_entity_system(
    ...
    let Some(active_path) = active_path_res else {
        return;
    };
    ...
    for ev in events.read() {
        if let TopologyMutation::Create(target, context_path) = ev {
            ...
            match target {
                ...
                CreateTarget::EnvRx { name, pos } => create_env_rx(
                    target_path,
                    name,
                    pos,
                    &mut graph,
                ...
                CreateTarget::EnvTx { name, pos } => create_env_tx(
                    target_path,
                    name,
                    pos,
                    &mut graph,
                ...
                CreateTarget::IoPin {
                    ...
                    crate::layout::systems::wm_file_ops::add_io_record(
                        &mut doc, section, name, &io_id, zone, 32, 32, None,
                    );
                    ...
                    if let Some(session) = graph.sessions.get_mut(target_path) {
                        ...
                        if let Some(io_data) = session.shard_io.get_mut(zone) {
                            if let Ok(new_io) =
                                toml::from_str::<node_editor::domain::ShardIoData>(&doc.to_string())
                            {
                                *io_data = new_io;
                            }
                        }
...
fn create_department(
    ...
    ui_states: &mut Query<&mut node_editor::domain::NodeGraphUiState>,
    _fs_cache: &layout_api::ProjectFsCache,
) {
    info!(
        "[Orchestrator] Starting clean birth of Department: {}",
        name
    );
    ...
    let sim_id = doc
        ...
        .unwrap_or("0000")
        .to_string();
    let sim_suffix = if sim_id.len() >= 4 {
        &sim_id[sim_id.len() - 4..]
    } else {
        "0000"
    };
    let dept_uuid = SystemMeta::generate().id;
    let dept_suffix = if dept_uuid.len() >= 4 {
        &dept_uuid[dept_uuid.len() - 4..]
    } else {
        "0000"
    };
    ...
    dept_table.insert(
        "depart_id_v1",
        Item::Value(toml_edit::Value::InlineTable(inline_id)),
    );
    ...
    if let Some(session) = graph.sessions.get_mut(active_path) {
        ...
        session
            .zone_ids
            .insert(name.to_string(), structured_dept_id.clone());
        session
            .node_inputs
            .insert(name.to_string(), vec!["in".to_string()]);
        session
            .node_outputs
            .insert(name.to_string(), vec!["out".to_string()]);
        session
            .layout_cache
            .insert(structured_dept_id, (pos.x, pos.y));
        session.is_dirty = true;
    }
    for mut ui in ui_states.iter_mut() {
        ui.node_positions.insert(name.to_string(), *pos);
    }
}
...
fn create_shard(
    ...
    let shard_suffix = if shard_uuid.len() >= 8 {
        &shard_uuid[shard_uuid.len() - 8..]
    } else {
        "00000000"
    };
    ...
    zone_table.insert(
        "blueprints",
        value(format!("{}/{}/blueprints.toml", dept_name, name)),
    );
    zone_table.insert(
        "anatomy",
        value(format!("{}/{}/anatomy.toml", dept_name, name)),
    );
...
"#,
    ...
    if let Some(session) = graph.sessions.get_mut(active_path) {
        ...
        session
            .zone_ids
            .insert(name.to_string(), structured_shard_id.clone());
        session
            .node_inputs
            .insert(name.to_string(), vec!["in".to_string()]);
        session
        ...
        session
            .layout_cache
            .insert(structured_shard_id, (pos.x, pos.y));
        session.is_dirty = true;
    }
    for mut ui in ui_states.iter_mut() {
        ui.node_positions.insert(name.to_string(), *pos);
...
fn create_connection(
    ...
    voxel_z: Option<u32>,
    graph: &mut BrainTopologyGraph,
    fs_cache: &layout_api::ProjectFsCache,
) {
    let is_from_rx;
    let is_to_tx;
    let from_id;
    ...
    {
        let session = if let Some(s) = graph.sessions.get(active_path) {
            s
        } else {
            return;
        };
    ...
    let from_pfx = if from_id.len() >= 4 {
        &from_id[from_id.len() - 4..]
    } else {
        &from_id
    };
    let to_sfx = if to_id.len() >= 4 {
        &to_id[to_id.len() - 4..]
    } else {
        &to_id
    };
    let uuid_full = SystemMeta::generate().id.replace("-", "");
    let core_id = if uuid_full.len() >= 6 {
        &uuid_full[..6]
    } else {
        &uuid_full
    };
    ...
    let src_shard_path = crate::layout::systems::wm_file_ops::find_path_by_id(fs_cache, &from_id)
        .unwrap_or_default();
    let dst_shard_path =
        crate::layout::systems::wm_file_ops::find_path_by_id(fs_cache, &to_id).unwrap_or_default();
    let src_io_path = src_shard_path
        .parent()
        .unwrap_or(Path::new("."))
        .join("io.toml");
    let dst_io_path = dst_shard_path
        .parent()
        .unwrap_or(Path::new("."))
        .join("io.toml");
    ...
    if is_from_rx {
                    ...
                    if let Some(pins) = matrix.get("pin").and_then(|p| p.as_array_of_tables()) {
                        for pin in pins.iter() {
                            if pin.get("name").and_then(|v| v.as_str()) == Some(from_port) {
                                proj_w =
                                    pin.get("width").and_then(|v| v.as_integer()).unwrap_or(32);
                                proj_h =
                                    pin.get("height").and_then(|v| v.as_integer()).unwrap_or(32);
                                break 'outer;
                            }
                        }
                    }
        ...
        let local_doc = match load_document(active_path) {
            Ok(d) => d,
            Err(_) => return,
        };
        //    :  ID
        let macro_path = if let Some(parent_id) = local_doc
            .get("depart_id_v1")
            .and_then(|i| i.get("id"))
            .and_then(|v| v.as_str())
        {
            crate::layout::systems::wm_file_ops::find_path_by_id(fs_cache, parent_id)
                .unwrap_or(active_path.to_path_buf())
        } else {
            active_path.to_path_buf()
        };

        let mut doc = match load_document(&macro_path) {
            Ok(d) => d,
            Err(_) => return,
        };
        ...
        if let Ok(mut dst_doc) = load_document(&dst_shard_path) {
            ...
            let current = dst_doc
                .get("settings")
                .and_then(|s| s.get("ghost_capacity"))
            ...
            info!(
                "[OK] [DCR] Reserved {} ghost_capacity for {}",
                capacity_add, to
            );
        }
    }
