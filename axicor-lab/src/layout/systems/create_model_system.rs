...
/// -:    
pub fn create_model_system(mut events: EventReader<CreateNewModelEvent>) {
    for ev in events.read() {
        ...
        if let Err(e) = fs::create_dir_all(&base_dir) {
            error!(
                "[ERROR] [Orchestrator] Failed to create model directory: {}",
                e
            );
            continue;
        }
...
"#,
        ...
        info!(
            " [Orchestrator] Model {} initialized. Further nodes must be added manually.",
            ev.model_name
        );
    }
}
