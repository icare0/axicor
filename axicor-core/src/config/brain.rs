...
pub struct ConnectionEntry {
    ...
    // TODO: In the future, this will contain a path to a connectome file (e.g. `baked/connections/v1_v2.ghosts`),
    // containing millions of connections. For MVP, we use a nested array.
    #[serde(default)]
...
mod tests {
    ...
    fn test_parse_brain_config() {
        ...
        assert_eq!(
            config.simulation.config.to_str().unwrap(),
        ...
        assert_eq!(
            config.zones[0].blueprints.to_str().unwrap(),
            "config/zones/V1/blueprints.toml"
        );
        assert_eq!(
            config.zones[0].anatomy.to_str().unwrap(),
            "config/zones/V1/anatomy.toml"
        );
        assert_eq!(
            config.zones[0].io.to_str().unwrap(),
            "config/zones/V1/io.toml"
        );
        assert_eq!(config.zones[0].baked_dir.to_str().unwrap(), "baked/V1/");

        assert_eq!(config.zones[1].name, "V2");
        assert_eq!(
            config.zones[1].blueprints.to_str().unwrap(),
            "config/zones/V2/blueprints.toml"
        );
        assert_eq!(
            config.zones[1].anatomy.to_str().unwrap(),
            "config/zones/V2/anatomy.toml"
        );
        assert_eq!(
            config.zones[1].io.to_str().unwrap(),
            "config/zones/V2/io.toml"
        );
