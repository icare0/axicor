...
impl EgressPool {
    pub fn new(capacity: usize) -> Self {
        ...
        for _ in 0..capacity {
            let mut buffer = Vec::with_capacity(genesis_core::constants::MAX_UDP_PAYLOAD);
            // [DOD FIX] Фиксируем длину. Мы больше НИКОГДА не будем делать push() или resize()
            unsafe { buffer.set_len(genesis_core::constants::MAX_UDP_PAYLOAD); }
