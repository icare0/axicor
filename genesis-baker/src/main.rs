...
use genesis_core::config::manifest::{ModelManifest, ManifestConnection};
...
fn bake_single_brain(brain_path: &Path, clean: bool, yes: bool) -> Result<()> {
    let brain_config = genesis_core::config::brain::parse_brain_config(brain_path)
...
fn bake_entire_model(model_path: &Path, clean: bool, yes: bool) -> Result<()> {
    ...
    let sim_config = genesis_core::config::SimulationConfig::load(model_path)
        .map_err(|e| anyhow::anyhow!(e))?;
    ...
    for dept in &sim_config.departments {
        ...
        if let Ok(brain_cfg) = genesis_core::config::brain::parse_brain_config(&brain_path) {
...
fn parse_and_validate(brain_config: &genesis_core::config::brain::BrainConfig, sim_path: &Path, bp_path: &Path, an_path: &Path, io_path: &Path, shard_cfg_path: &Path, out_dir: &Path, zone_name: &str, zone_idx: u16) -> Result<BakeWorkspace> {
    ...
    let shard_cfg = genesis_core::config::InstanceConfig::load(shard_cfg_path).map_err(anyhow::Error::msg)?;
