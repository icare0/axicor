...
- **Absolute unit:** 1.0 (float) = **1 µm** (micrometer).
...
- **Relative unit (Normalization):** 0.0 ... 1.0 (pct).
  ...
  - Logic: The L1 layer can occupy 0.0 - 0.1 (10% of height). This value is multiplied by `world.height` (in µm) during initialization. This allows resizing the "brain" without rewriting the layer architecture.
- **Discretization (Grid):** Space is quantized into **voxels** (size is specified in the configuration, e.g., 10 µm).
...
- **Parameter:** `segment_length_voxels` (e.g., 4 voxels = 100 µm with a 25 µm voxel).
- **Logic:**
...
- **Optimization:** Reduces memory consumption by `segment_length_voxels` times. Instead of 1000 records per 1 mm of path (at 1 µm resolution) - only **10 records** (with a 100 µm segment).
...
- **Resolution:** `tick_duration_us` = 100 (0.1 ms).
  - Rationale: 1 ms is too "coarse" for GSOP (spike order is lost). 100 µs is the bare minimum for causality.
...
We move math out of the Hot Loop. We write human-readable quantities (m/s, µm, ms) in the configs, but the engine recalculates them into "raw" constants for the GPU at startup.
...
<tr>
  ...
  <td><code>0.5 m/s × 100µs = 50 µm/tick</code></td>
...
| :--- | :--- | :--- |
| `tick_duration_us` | 100 | µs (0.1 ms) |
| `voxel_size_um` | 25 | µm |
| `segment_length_voxels` | 2 | voxels (= 50 µm) |
| `signal_speed_um_tick` | 50 | µm/tick (= 0.5 m/s) |
| `v_seg` | 1 | segment/tick |

- Verification: 50 µm/tick ÷ 50 µm/segment = 1 ✓ (integer).
...
1. **Segment size in micrometers:** `segment_length_um = voxel_size_um × segment_length_voxels`
   `segment_length_um = 25 µm × 2 = 50 µm`
2. **Discrete speed (segments/tick):** `v_seg = signal_speed_um_tick ÷ segment_length_um`
   `v_seg = 50 µm/tick ÷ 50 µm/segment = 1 segment/tick`
...
- In `simulation.toml`: `signal_speed` = 0.5 m/s, `tick_duration` = 100 µs
- Engine calculates: `signal_speed_um_tick` = 50 µm/tick
...
- Engine calculates: `segment_length_um` = 50 µm
- Then: `v_seg` = 50 µm/tick ÷ 50 µm = 1
