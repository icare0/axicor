...
| :--- | :--- | :--- | :--- | :--- |
| `[7:4]` | `type_mask` | u4 | Neuron type index (0..15). Used as a direct index into `VARIANT_LUT`. | ✅ Active |
| `[3:1]` | `burst_count` | u3 | [BDP] Serial spike counter (0..7). Incremented on every spike within a single batch. | ✅ Active |
| `[0:0]` | `is_spiking` | u1 | Instant Spike. 1 = soma generates a spike in the current tick. | ✅ Active |
