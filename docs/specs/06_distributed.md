...
**Dynamic MTU:** Константа `MAX_EVENTS_PER_PACKET` удалена. Теперь лимит спайков вычисляется динамически для каждого пира на основе значения `mtu`, полученного через `RouteUpdate`:
`max_events_per_packet = (peer_mtu - 16) / 8`.

Где:
- `16` — размер `SpikeBatchHeaderV2`.
- `8` — размер одного `SpikeEventV2`.
- **PC Default:** MTU = 65507 (8186 спайков).
- **ESP32 Default:** MTU = 1400 (173 спайка).
