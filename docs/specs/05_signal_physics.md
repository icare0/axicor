...
Движок разделяет синаптический вес на две физические сущности: **структурную массу** (для обучения) и **электрический заряд** (для динамики мембраны).

#### 1.3.1. Разделение Массы и Заряда (Fixed-Point Downscale)

Этот архитектурный паттерн позволяет совместить огромный динамический диапазон пластичности с биологически точной силой импульсов.

*   **Mass Domain (i32):** Синаптический вес хранится и изменяется в диапазоне до **2,140,000,000**. Все STDP-градиенты (GSOP) применяются напрямую к этой массе. Это позволяет моделировать накопление микро-опыта, который не виден в электрической активности, но критичен для долгосрочной стабильности.
*   **Charge Domain (16-bit shift):** В горячем цикле ядра `UpdateNeurons` масса конвертируется в электрический заряд (микровольты) одним арифметическим сдвигом: `int32_t charge = w >> 16;`. 

**Эффекты:**
1.  **Сверхтонкие градиенты:** Один спайк может изменить массу на 80 единиц, что составляет всего **~0.00002%** от общего диапазона. Это гарантирует монументальную стабильность навыков.
2.  **Немые синапсы (Silent Synapses):** Синапс может иметь массу (например, 30,000), но его заряд после сдвига будет равен **0**. Такая связь структурно существует, обучается, но не влияет на мембрану сомы, пока не накопит критическую массу для "прорыва" в электрический домен.
...
5. **Двухфакторный STDP:**
   - Вычисляем ранг инерции синапса: `rank = abs(weight) >> 27`.
