...
> Часть архитектуры [Genesis](../../design_specs.md). Единая абстракция ввода/вывода и межзональных связей.
...
### 2.4. Множественные матрицы и Мульти-Резолюция

На одну зону может быть подключено **N входных матриц** произвольных размеров:
...
Baker конкатенирует все виртуальные аксоны в один плоский массив:

```
axons[0 .. 4095]                = "vertical_edges" (64×64)
axons[4096 .. 2_077_695]        = "color_diff"     (1920×1080)
axons[2_077_696 .. 2_079_495]   = "motion_field"   (36×50)
```

> **Инвариант: Порядок конкатенации = порядок объявления** блоков `[[input]]` в `io.toml`. Перестановка блоков местами **ломает все ID**. Baker записывает offset'ы в `.gxi` файл.

Движок видит только общий `Input_Bitmask` — ему неизвестно понятие «feature layer».
...
pub struct ExternalIoHeader {
    pub zone_hash: u32,      // FNV-1a hash имени zone (e.g., "SensoryCortex")
    pub matrix_hash: u32,    // FNV-1a hash matrix name (e.g., "retina") или 0 для default
    pub payload_size: u32,   // Размер payload в байтах (без заголовка)
}

// Payload: Input_Bitmask данные
// Размер: ceil(total_virtual_axons / 32) × 4 × effective_ticks байт
```
...
### 2.8. Хостовая конвенция: Feature Layers по тикам

> **Заметка для IDE:** Ниже описана рекомендуемая конвенция, а не часть движка.

Если хост подаёт разные feature layers в разные тики, рекомендуемый паттерн:

```
Тик 0: зажечь биты "vertical_edges" (low-res оконтуривание)
Тик 1: зажечь биты "color_diff"     (high-res цвет)
Тик 2: зажечь биты "motion_field"   (low-res motion vectors)
Тик 3-199: нули (мозг переваривает)
```

Каждая матрица активируется **в свой тик**. GSOP обучит дендриты различать слои по моменту контакта (временнóе кодирование).

Альтернативный паттерн: **все слои одновременно** в тик 0.

Движку не важно — он видит биты. Интерпретация = ответственность хоста.
